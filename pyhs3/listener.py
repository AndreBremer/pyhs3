...
class ASCIIListener:
    ...
    async def _start_listener(self):
        ...
        try:
            ...
            _LOGGER.info(
                "HomeSeer ASCII Listener connected to {}:{}".format(
    ...
    async def _login(self):
        ...
        _LOGGER.debug(
            "HomeSeer ASCII logging in with user {} and pass {}".format(
    ...
    async def _handle_message(self, raw):
        ...
        if msg[0] == "DC" and self._async_message_callback is not None:
            ...
            _LOGGER.debug(
                "HomeSeer unhandled ASCII message type received: {}".format(
                    msg[0].strip()
                )
            )
    ...
    async def connection_handler(self):
        if self.state == STATE_STOPPED:
            ...
            if self._reconnect_flag:
                time = 10
                _LOGGER.info(
                    "Reconnecting ASCII Listener in {} seconds...".format(time)
                )
                await asyncio.sleep(time)
            _LOGGER.debug(
                "Connecting ASCII listener to {}:{}".format(self._host, self._port)
            )
            self._state = STATE_IDLE
