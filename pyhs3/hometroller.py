...
class HomeTroller:
    ...
    async def _request(self, method, params=None, json=None):
        ...
        try:
            async with self._websession.request(
                ...
                _LOGGER.debug(
                    "HomeSeer request response: {}".format(await result.text())
                )
                return await result.json()

        except ContentTypeError:
            _LOGGER.debug(
                "HomeSeer returned non-JSON response: {}".format(await result.text())
            )

        except TimeoutError:
            _LOGGER.error(
                "Timeout while requesting HomeSeer data from {}:{}".format(
    ...
    async def _update_device_value(self, device_ref, value):
        try:
            ...
            _LOGGER.debug(
                "HomeSeer device '{}' ({}) updated to: {}".format(
                    device.name, device.ref, device.value
                )
            )
        except KeyError:
            _LOGGER.debug(
                "HomeSeer update received for unsupported device: {}".format(device_ref)
            )
    ...
    async def refresh_devices(self, reason=None):
        ...
        for device in all_devices:
            try:
                ...
                _LOGGER.debug(
                    "HomeSeer device '{}' ({}) refreshed to: {}".format(
                        dev.name, dev.ref, dev.value
                    )
                )
            except KeyError:
                _LOGGER.debug(
                    "HomeSeer refresh data retrieved for unsupported device type: {} ({})".format(
